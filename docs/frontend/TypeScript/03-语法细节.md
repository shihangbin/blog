...
`1`. 联合类型是由两个或者多个其他类型组成的类型； `2`. 联合类型中的每一个类型被称之为联合成员（`union`'`s members`）； `3`. 我们拿到的值可能是 `string` 或者 `number`，我们就不能对其调用 `string` 上的一些方法； `4`. 我们需要使用 `TypeScript` 缩小的代码结构，推断出更加具体的类型 `5`. 联合类型使用 `|` 符号
...
`1`. 交叉类似表示需要满足多个类型的条件 `2`. 交叉类型使用 `&` 符号
