...
`TypeScript` 使用模式命名这些声明文件 `lib.[something].d.ts`。 ![](https://img.xbin.cn/images/2023/08/15-17-42-3c55b9.png)
...
`1`. 创建 `service` 文件夹 `2`. `service` 里面创建 `config` & `modules` & `request` 文件夹 `3`. `service` 里面创建 `index.ts` 文件
