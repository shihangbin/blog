...
`1`. `v-enter-from`：进入动画的起始状态。 `2`. `v-enter-active`：进入动画的生效状态。 `3`. `v-enter-to`：进入动画的结束状态。 `4`. `v-leave-from`：离开动画的起始状态。 `5`. `v-leave-active`：离开动画的生效状态。 `6`. `v-leave-to`：离开动画的结束状态。
...
```js
<Transition name='dsj'>...</Transition>
```
