...
所有表均遵循以下设计规范： `1`. **主键**：`id` (`BIGINT`)，采用 **`AUTO_INCREMENT`** 自增策略。 `2`. **逻辑删除**：包含 `is_deleted` 字段 (`0`=正常, `1`=删除)，`MyBatis-Plus` 自动处理。 `3`. **时间戳**：包含 `create_time` 字段，默认当前时间。
...
![1768054753506](https://img.xbin.cn/blog/20260406133948214.png)
