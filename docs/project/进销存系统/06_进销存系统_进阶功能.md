...
`1`. **`Excel` 导出**：使用 `EasyExcel` 导出订单报表。 `2`. **定时任务**：实现库存不足时的自动预警。
...
`1`. **定义实体类**：使用 `@ExcelProperty` 注解定义表头。 `2`. **调用写方法**：`EasyExcel.write(fileName, Class).sheet("sheetName").doWrite(dataList);`
...
**使用步骤：** `1`. **开启支持**：在启动类添加 `@EnableScheduling`。 `2`. **定义任务**：在方法上添加 `@Scheduled` 注解。
