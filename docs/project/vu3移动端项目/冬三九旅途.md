...
`1`. `npm i` `2`. `npm i scss` -`D` && `npm i sass` -`D` `3`. `npm run dev` | `npm run build` `4`. 修改 `publlic` => `favicon.io` `5`. 修改 `index.html` => `title` `6`. `css` 样式重置(`normalize.css` && `reset.css`)
...
<template>
  <div class="tab-bar">
    <template v-for="(item, index) in tabbarData" :key="index">
      ...
      >
        <img v-if="currentIndex !== index" :src="getAssetURL(item.image)" alt="" class="img" />
        <img v-else :src="getAssetURL(item.imageActive)" alt="" class="img" />
...
`1`. 用插槽 => 直接在局部中修改元素 `2`. 全局定义一个变量 => `assets`
...
<div class="city">
  <div class="top">
    ...
    <!-- tab切换 -->
    <van-tabs v-model:active="cityActive" color="#45cfff">
  ...
  <div class="content">
    ...
    <template v-for="(value, key, index) in allcities" :key="key">
      <div v-show="cityActive === key">
        <city-group v-show="cityActive === key" :groupData="value"></city-group>
      </div>
    </template>
  </div>
</div>
...
<div class="city-group">
  <van-index-bar highlight-color="#45cfff" :index-list="indexList">
    <!-- 热门 -->
    <van-index-anchor index="#">热门</van-index-anchor>
    <div class="hot">
      <template v-for="(hot, index) in groupData.hotCities" :key="index">
        <div class="hot-item" @click="onHotCity(hot)">{{ hot.cityName }}</div>
      </template>
    </div>
    <!-- 列表 -->
    <template v-for="(group, index) in groupData.cities" :key="index">
      ...
      <template v-for="(city, index) in group.cities" :key="index">
        <van-cell :title="city.cityName" @click="onHotCity(hot)" />
      </template>
    </template>
  </van-index-bar>
</div>
