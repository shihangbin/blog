...
>
> 当前版本：3.5.17只要保证接口中参数名和#{}中指定的名称一致，可以省略@Param注解
>
...
| 延迟加载[低版] | lazyLoadingEnabled设置为`true`<br />aggressiveLazyLoading设置为`false` | MyBatis 全局配置文件中的 settings<br />和 Spring Boot 整合后就是 application.properties |
| 延迟加载[高版] | lazyLoadingEnabled设置为`true` | MyBatis 全局配置文件中的 settings<br />和 Spring Boot 整合后就是 application.properties |
...
3. 全局配置替代：若所有表都有统一前缀（如t\_），可在application.properties中配置全局表前缀，避免每个实体类写@TableName：
