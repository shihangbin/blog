...
文本块就是指多行字符串，例如一段格式化后的 `XML`、`JSON` 等。而有了文本块以后，用户不需要转义，`Java` 能自动搞定。因此，**文本块将提高 `Java` 程序的可读性和可写性。**{`username`:"`tom`",`password`="`111`"}
...
（`2`）允许开发人员使用“\`n`”“\`f`”和“\`r`”来进行字符串的垂直格式化，使用“\`b`”“\`t`”进行水平格式化。如以下示例代码就是合法的。
...
> `String`,`StringBuilder` 以及 `StringBuffer` 区别: 背下来
>
> `1`.相同点:
>
> ​ 三者都可以拼接字符串
