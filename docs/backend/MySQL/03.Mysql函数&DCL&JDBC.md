...
| 函数 | 用法 |
| --- | --- |
| `IF`(比较,`t` ,`f`)<br>里面的 `t` 和 `f` 是两个结果 | 如果比较是真，返回 `t`，否则返回 `f` |
| `IFNULL`(`value1`, `value2`) | 如果 `value1` 不为空，返回 `value1`，否则返回 `value2` |
| `CASE WHEN` 条件 `1 THEN result1 WHEN` 条件 `2 THEN result2 ....` [`ELSE resultn`] `END` | 相当于 `Java` 的 `if`...`else if`...`else...` |

- 练习 `1`：获取用户的姓名、性别，如果性别为 `1` 则显示'男'，否则显示'女'；要求使用 `if` 函数查询：

  ```sql
  SELECT uname,IF(sex=1,'男','女') sex FROM t_user;
  ```

- 练习 `2`：获取用户的姓名、性别，如果性别为 `null` 则显示为 `0`；要求使用 `ifnull` 函数查询：

  ```sql
  SELECT uname,IFNULL(sex,0) sex FROM t_user;
  ```

- 练习 `3`：如果 `age`<=`12`,显示儿童,如果 `age`<=`18`,显示少年,如果 `age`<=`40`,显示中年,否则显示老年
...
我们现在默认使用的都是 `root` 用户，超级管理员，拥有全部的权限。但是，一个公司里面的数据库服务器上面可能同时运行着很多个项目的数据库。所以，我们应该根据不同的项目建立不同的用户，分配不同的权限来管理和维护数据库。

<img src="https://img.xbin.cn/blog/20260405171039036.png" alt="image-20260206102646174" style="zoom:80%;" />
...
用户创建之后，基本没什么权限！需要给用户授权 ![](https://img.xbin.cn/blog/20260405171039037.png)
...
![](https://img.xbin.cn/blog/20260405171039038.png)
...
![](https://img.xbin.cn/blog/20260405171039039.png)
...
- 撤销 `user1` 用户对 `test` 操作的权限
  ...
  ![](https://img.xbin.cn/blog/20260405171039040.png)
...
- 查看 `user1` 用户的权限
  ...
  ![](https://img.xbin.cn/blog/20260405171039041.png)
...
- 删除 `user2`
  ...
  ![](https://img.xbin.cn/blog/20260405171039042.png)
...
![](https://img.xbin.cn/blog/20260405171039043.png)
...
![](https://img.xbin.cn/blog/20260405171039044.png)
...
<img src="https://img.xbin.cn/blog/20260405171039045.png" alt="image-20260206104331483" style="zoom:80%;" />

### `2.JDBC` 准备(导入 `jdbc` 依赖)
...
> 其实我们不需要注册驱动,因为现在 `jdbc` 可以自动注册,但是为了迎合后面课程的配置文件中的配置,我们加上注册驱动
...
> 我们的 `sql` 语句可以先在 `sqlyog` 中写一遍测一测,然后再粘贴到咱们得 `java` 代码中
...
## 第四章.`PreparedStatement` 预处理对象

### `1.sql` 注入的问题以及解决方式(预处理对象)
...
<img src="https://img.xbin.cn/blog/20260405171039047.png" alt="image-20260206153208422" style="zoom:80%;" />

> 密码输入:`222`' `or` '`1`' = '`1` 以上程序不行了->`sql` 注入
...
<img src="https://img.xbin.cn/blog/20260405171039048.png" alt="image-20260206154043865" style="zoom:80%;" />
...
> ​ 如果有,注册失败
>
> ​ 否则,将用户名和密码 `insert` 到数据库中,显示注册成功
