...
| CASE WHEN 条件1 THEN result1 WHEN 条件2 THEN result2 .... [ELSE resultn] END | 相当于Java的`if`...`else` `if`...`else`... |

- 练习1：获取用户的姓名、性别，如果性别为1则显示'男'，否则显示'女'；要求使用`if`函数查询：

  ```sql
  SELECT uname,IF(sex=1,'男','女') sex FROM t_user;
  ```

- 练习2：获取用户的姓名、性别，如果性别为`null`则显示为0；要求使用ifnull函数查询：
